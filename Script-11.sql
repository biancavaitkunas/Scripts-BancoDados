...
(12, 3, 5, 4);
INSERT INTO venda_produto values (13, 1, 2, 5);
...
and cidade.nome = 'Campinas';

-- 9 - SQL para listar vendas por cliente
select venda.id, venda.dt_venda
from venda
inner join cliente on cliente.id = venda.cliente
and cliente.id = 2;

-- 10 - SQL com o tipo de join mais adequado para listar todos os produtos e categorias
select produto.nome, categoria.nome
from produto
right join categoria on categoria.id = produto.categoria

-- 11 - Listar quantidade de vezes que cada produto foi vendido, junto com a quantidade total de produtos vendidos
select produto.nome, count (venda.*) as qtd_vezes, sum (qtd_vendida)as qtd_produto
from produto
inner join venda_produto vp on vp.produto = produto.id 
inner join venda on venda.id = vp.venda
group by produto.nome;

-- 12 - Listar faturamento de cada cliente
select cliente.nome, sum (valor)as fat
from cliente
inner join venda on venda.cliente = cliente.id 
inner join venda_produto vp on vp.venda = venda.id
inner join produto on produto.id = vp.produto
